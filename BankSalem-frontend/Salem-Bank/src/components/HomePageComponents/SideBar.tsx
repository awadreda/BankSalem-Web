  ...
  const drawer = (
    <Box sx={{ overflow: "auto", mt: 8 }}>
      <Typography
        variant="h6"
        align="center"
        sx={{ fontWeight: "bold", fontSize: "1.5rem", mb: 2, color: "white" }}
      >
        {currentUser?.firstName} {currentUser?.lastName}
      </Typography>
      <List>
        {menuItems.map((item) => {
          // Only show Admin menu item if user has admin permission (Premsion === 1)
          if (item.text === "Admin" && currentUser?.permission !== 1) {
            return null;
          }
